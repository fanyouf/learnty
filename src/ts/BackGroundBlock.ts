...
class BGBlocks implements Actor {
  ...
  draw(ctx:CanvasRenderingContext2D){
    ctx.fillStyle = "green";
      this.matrix.forEach((row, y) => {
        row.forEach((val, x) => {
          if(val){
            ctx.fillRect(x * this.unitBlockh, y * this.unitBlockh, this.unitBlockh-1, this.unitBlockh-1);
          }
          else{
            ctx.strokeStyle = "green";
            ctx.strokeRect(x * this.unitBlockh, y * this.unitBlockh, this.unitBlockh, this.unitBlockh);
          }
        });
      });
  }
  ...
  clearSomeRow(){
    let arr = this.matrix.map(row=> row.reduce((init, current) => init+current, 0) )
